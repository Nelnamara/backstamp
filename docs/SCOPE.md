...
# Backstamp — Scope Doc
*Name locked: **Backstamp** — the mark on the reverse of a pin/badge that proves its maker and authenticity. Specific to the pin/Blizzard-trading crowd, and echoes the backstamp/hallmark fake-alert reference already in Pillar 1. Last synced with conversation: 2026-08-04.*
...
- Tracks **purchase price and current market value** (collection worth over time, not just a static catalog). **Decided 2026-08-04: value is a first-class, visible metric throughout the UI, including the main catalog view — needed for overall collection worth and for insurance tracking.** (The aggregate-total privacy rule below is unchanged: the collection total still renders only on the owner's private dashboard.)
- **Multiple photos per item** — item itself, box/packaging, certificate of authenticity/numbering, condition shots. **Decided 2026-08-04: at least one photo is required on every item — every entry is a real, photographed object.**
- **Trade/sale listing photos carry verification (decided 2026-08-04):** when an item is offered for trade or sale, its listing image must show a date and the seller's username — possibly stamped onto the image by the app itself (visible timestamp + user ID) at upload. Mechanism ("possibly we provide") not final; the requirement is. Note: this is a *visible* overlay for verification — the existing GPS/EXIF stripping of *invisible* metadata still applies first.
...
- **Wishlist priority tags — Grail / Filler (adopted 2026-08-04):** collector-vocabulary priority labels on wishlist entries, adopted from the Claude Design pass during the UI merge. Needs a small schema addition (wishlist entries currently have no priority field) — flagged, not yet built.
...
- **Frontend:** hosted web app (PWA) to start, with browser-based camera capture for photo intake. Native **iOS app next**, **Android after that**, both against the same FastAPI REST API. **Decided 2026-08-04: built with React (Vite).**
- **UI direction (decided 2026-08-04, from two independent blind design passes that converged on the same palette family):** the **Display Case** — dark walnut/brass, photo-first — as the app's identity, carrying the **Registry's organizational layer** throughout (global catalog numbers, mono spec lines) with a cream accession-card as the item-detail view ("pulled into the lamplight"). Desktop/iPad is the design target; the phone adaptation is the Claude Design pass's bottom-tab frame. Also adopted from that pass: verified/redeemed status badges in the catalog, Grail/Filler wishlist tags, and its Connect tier-selector sketch as the eventual Pillar-3 direction. The approved mock board is the build spec.
